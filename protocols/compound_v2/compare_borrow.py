"""Compare Compound V2 borrow data between subgraph and DeFiLlama.
...
def main():
    ...
    with open(BASE / "compound_info.csv", newline="") as f:
        ...
        next(reader)
        for item in reader:
            graph[item[1]] = (int(item[0]), float(item[2]))
            dates.append(item[1])
    ...
    with open(BASE / "compound-defillama.csv", newline="") as f:
        ...
        for item in reader:
            if len(item) > 4 and item[4]:
                try:
                    llama[item[1]] = (int(item[2]), float(item[4]))
                except ValueError:
                    pass
    ...
    print("Max borrow difference:", max(diffs, key=lambda x: x[1]))
