"""Process AAVE daily snapshot data: add utilization rate and liquidity columns.

Reads:  aave.csv
Writes: aave_info.csv
"""
...
def main():
    rows = []
    with open(BASE / "aave.csv", newline="") as f:
        reader = csv.reader(f)
        next(reader)
        for item in reader:
            tvl = float(item[2])
