...
"""
    Get all swap transaction from AAVE V2 subgraph
"""
import csv
import requests
...
while True:
    query = """
    {
        swaps(where: {timestamp_lte: """ + str(current_time) + """, timestamp_gt: """ + START_TIME + """}, first: 1000, orderBy: timestamp, orderDirection: desc) {
            id
            pool {
                id
            }
